...
if (!$sampleMode && isset($_GET['logout'])) {
    session_destroy();
    header('Location: /admin-honeymoon-fund');
    exit;
}
    ...
    try {
        ...
        if ($amount <= 0) {
            ...
            if ($contributionId) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE honeymoon_fund_contributions
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO honeymoon_fund_contributions (amount, contributor_name)
                    VALUES (?, ?)
                ");
                $stmt->execute([
                    $amount,
...
if (!$sampleMode && $authenticated && isset($_GET['toggle_visibility'])) {
    try {
        $pdo = getDbConnection();
        $stmt = $pdo->prepare("SELECT setting_value FROM site_settings WHERE setting_key = 'honeymoon_fund_visible'");
        $stmt->execute();
        $row = $stmt->fetch();
        $currentValue = $row ? $row['setting_value'] : '1';
        $newValue = $currentValue === '1' ? '0' : '1';
        $pdo->prepare("INSERT INTO site_settings (setting_key, setting_value) VALUES ('honeymoon_fund_visible', ?) ON DUPLICATE KEY UPDATE setting_value = ?")->execute([$newValue, $newValue]);
        $success = $newValue === '1' ? 'Honeymoon fund is now visible on the registry page.' : 'Honeymoon fund is now hidden from the registry page.';
    } catch (Exception $e) {
...
if (!$sampleMode && $authenticated && isset($_GET['delete'])) {
    try {
        $pdo = getDbConnection();
        $stmt = $pdo->prepare("DELETE FROM honeymoon_fund_contributions WHERE id = ?");
...
if ($sampleMode && isset($_GET['edit'])) {
    foreach (getSampleHoneymoonFundContributions() as $sampleContribution) {
    ...
    try {
        $pdo = getDbConnection();
        $stmt = $pdo->prepare("SELECT * FROM honeymoon_fund_contributions WHERE id = ?");
...
$fundVisible = true;
if ($sampleMode) {
    $contributions = getSampleHoneymoonFundContributions();
...
$page_title = "Manage Honeymoon Fund - Jacob & Melissa";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <main class="page-container">
        <div class="admin-container">
            <?php renderAdminSampleBanner('Honeymoon Fund Sample Mode'); ?>
            ...
            <?php if (!$authenticated): ?>
                <div class="form-container">
                    <h1 class="page-title">Manage Honeymoon Fund</h1>
                    <?php if ($error): ?>
                        <div class="alert alert-error">
                            <p><?php echo htmlspecialchars($error); ?></p>
                        </div>
                    <?php endif; ?>
                    <form method="POST" action="/admin-honeymoon-fund">
            ...
            <?php else: ?>
                <div class="admin-header">
                    <h1 class="page-title">Manage Honeymoon Fund</h1>
                    <div>
                        <a href="/admin-honeymoon-fund?logout=1" class="btn btn-secondary">Logout</a>
                    </div>
                </div>
                ...
                <div class="visibility-toggle">
                    ...
                    <a href="/admin-honeymoon-fund?toggle_visibility=1" class="btn btn-small <?php echo $fundVisible ? 'btn-hide' : 'btn-show'; ?>">
                ...
                <div class="form-container">
                    ...
                    <form method="POST" action="/admin-honeymoon-fund">
                        ...
                        <div class="form-actions">
                            ...
                            <?php if ($editContribution): ?>
                                <a href="/admin-honeymoon-fund" class="btn btn-secondary">Cancel</a>
                            <?php endif; ?>
                        </div>
                    </form>
                </div>
                                ...
                                <?php foreach ($contributions as $contribution): ?>
                                    <tr>
                                        ...
                                        <td class="actions-cell">
                                            <a href="/admin-honeymoon-fund?edit=<?php echo $contribution['id']; ?>" class="btn btn-small btn-edit">Edit</a>
                                            <a href="/admin-honeymoon-fund?delete=<?php echo $contribution['id']; ?>" class="btn btn-small btn-delete" onclick="return confirm('Are you sure you want to delete this contribution?');">Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
